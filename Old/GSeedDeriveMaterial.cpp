...
void GSeedDeriveMaterial::Reset() {
    ...
    mUnwindA = VarSymbol("aUnwindA");
    mUnwindB = VarSymbol("aUnwindB");
    mUnwindC = VarSymbol("aUnwindC");
    mUnwindD = VarSymbol("aUnwindD");
...
bool GSeedDeriveMaterial::Build(TwistProgramBranch &pBranch,
    ...
    auto AppendLaneLoop = [&](GWhile *pWhile,
        ...
        std::vector<GSymbol> aUnwinds;
        aUnwinds.push_back(mUnwindA);
        aUnwinds.push_back(mUnwindB);
        aUnwinds.push_back(mUnwindC);
        aUnwinds.push_back(mUnwindD);
        Random::Shuffle(&aUnwinds);
    ...
    for (int i = 0; i < SEED_WORK_LANE_COUNT; i++) {
        ...
        // Farm passes are now single-shot derive rounds.
        aStatements.push_back(GStatement::Assign(GTarget::Symbol(aReloop), GExpr::Const64(0ULL)));
