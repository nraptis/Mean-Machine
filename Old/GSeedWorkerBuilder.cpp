...
void ConfigureLoop(GLoop *pLoop,
    ...
    pLoop->mLoopVariableName = pLoopIndex.IsVar() ? pLoopIndex.mName : "aIndex";
    pLoop->mLoopBegin = 0;
...
bool GSeedWorkerBuilder::BuildPhaseA(TwistProgramBranch &pBranch,
    ...
    // aValueMix = (aValue + aCarry) ^ aSBoxA[aIndex & 0xFFU];
    // aValue = TwistMix64::Mix161_004(aValueMix, aSBoxB);
    ...
    aComposerA.MixVariableAdd(mValue);
    aComposerA.MixVariableXor(mCarry);
    aComposerA.MixBuffer(mSBoxA)
        .Key(mLoopIndex)
        .Domain(GLoopFragmentDomain::kSBox)
        .Offset(0);

    if (!aComposerA.Bake(&aLoopA.mBodyStatements, pErrorMessage)) {
        return false;
    }
    ...
    aLoopA.mBodyStatements.push_back(
        GStatement::Assign(
            ...
            GExpr::Mix161(GExpr::Symbol(aValueMix), Mix161Type::kMix161_001, mSBoxB)));
    ...
    aComposerB.MixVariableXor(mCarry);
    ...
    aLoopB.mBodyStatements.push_back(
        GStatement::Assign(
            GTarget::Symbol(mValue),
            GExpr::Mix161(GExpr::Symbol(aValueMix), Mix161Type::kMix161_000, mDerivedSBoxA)));
...
bool GSeedWorkerBuilder::ExecuteBranch(TwistWorkSpace *pWorkspace,
    ...
    if (pWorkspace == nullptr) {
        SetError(pErrorMessage, "GSeedWorkerBuilder::ExecuteBranch received a null workspace.");
        return false;
    }
    ...
    auto ExecuteBatchByIndex = [&](const std::size_t pBatchIndex) -> bool {
        ...
        if (!GBatch::FromJson(aBatchJson, &aBatch, pErrorMessage)) {
            if ((pErrorMessage != nullptr) && pErrorMessage->empty()) {
                *pErrorMessage = "Failed to parse branch batch JSON in ExecuteBranch.";
            }
            return false;
        }

        if (!aBatch.ExecuteWithVariables(pWorkspace, pExpander, &aVariables, pErrorMessage)) {
            if ((pErrorMessage != nullptr) && pErrorMessage->empty()) {
                *pErrorMessage = "Failed to execute branch batch.";
            }
            return false;
        }
        return true;
    ...
    if (aSteps.empty()) {
        for (const std::string &aLine : pBranch.GetStringLines()) {
            ...
            const bool aExecutedCrypto = ExecuteCryptoMakeLine(aLine, pWorkspace, pExpander, &aLineError);
            if (!aLineError.empty()) {
                SetError(pErrorMessage, "GSeedWorkerBuilder branch line failed: " + aLineError);
                return false;
            }
