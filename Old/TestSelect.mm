...
- (void)testSelectStatementHelloWorld {
    ...
    std::vector<std::string> expected = {
        ...
        "\tmSaltAKeyA = i + 100U;",
        "\tmSaltBKeyA = i + 25U;",
        "\taCarry = aCarry ^ (mSaltA[(mSaltAKeyA & 0x7FU)] + mSaltB[(mSaltBKeyA & 0x7FU)]);",
        "} else {",
        "\taCarry = aCarry ^ mSaltD[(aValue & 0x7FU)];",
        "}"
    };
...
- (void)testSelectStatementJsonRoundTripRuntime {
    ...
    std::unordered_map<std::string, GRuntimeScalar> aVarsA;
    ...
    std::unordered_map<std::string, GRuntimeScalar> aVarsB;
...
- (void)testFastMatrixBakeAndExecute {
    ...
    std::unordered_map<std::string, GRuntimeScalar> aRuntimeVars;
