...
void GSeedMatrixRollups::Reset() {
    ...
    mListWorkers.push_back(BufSymbol(TwistWorkSpaceSlot::kWorkLaneE));
    mListWorkers.push_back(BufSymbol(TwistWorkSpaceSlot::kWorkLaneF));
    ...
    mListExpansion.push_back(BufSymbol(TwistWorkSpaceSlot::kSeedExpansionLaneE));
    mListExpansion.push_back(BufSymbol(TwistWorkSpaceSlot::kSeedExpansionLaneF));
    ...
    mListOperationLanes.push_back(BufSymbol(TwistWorkSpaceSlot::kOperationLaneC));
    mListOperationLanes.push_back(BufSymbol(TwistWorkSpaceSlot::kOperationLaneD));
    mListOperationLanes.push_back(BufSymbol(TwistWorkSpaceSlot::kOperationLaneE));
    mListOperationLanes.push_back(BufSymbol(TwistWorkSpaceSlot::kOperationLaneF));
...
bool GSeedMatrixRollups::Build(TwistProgramBranch &pBranch,
    ...
    auto SelectPhaseOpLanes = [&]() -> std::pair<GSymbol, GSymbol> {
        const std::size_t aLaneCount = mListOperationLanes.size();
        if (aLaneCount < 2U) {
            return std::make_pair(BufSymbol(TwistWorkSpaceSlot::kOperationLaneA),
                                  BufSymbol(TwistWorkSpaceSlot::kOperationLaneB));
        }
        const std::size_t aFirstIndex = static_cast<std::size_t>(Random::Get(static_cast<int>(aLaneCount)));
        std::size_t aSecondIndex = static_cast<std::size_t>(Random::Get(static_cast<int>(aLaneCount - 1U)));
        if (aSecondIndex >= aFirstIndex) {
            ++aSecondIndex;
        }
        return std::make_pair(mListOperationLanes[aFirstIndex], mListOperationLanes[aSecondIndex]);
    };
