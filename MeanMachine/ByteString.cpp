...
void ByteString::Set(const std::uint8_t *pString, int pLength) {
    ...
    // If source overlaps and we might reallocate, copy first
    if (pLength > mSize) {
        std::uint8_t *aTemp = new std::uint8_t[pLength];
        std::memmove(aTemp, pString, pLength);

        Size(pLength);  // may free old mData

        std::memcpy(mData, aTemp, pLength);
        delete [] aTemp;
    } else {
        std::memmove(mData, pString, static_cast<std::size_t>(pLength));
    }
