...
//  MeanMachine
//
//  Created by Nick Raptis on 5/25/26.
//
...
bool GSeedRunKDF3::Plan(std::string *pErrorMessage) {
    if (pErrorMessage != nullptr) {
        pErrorMessage->clear();
    }
    ...
    if (!aAXSK->Bake(GAXSFormat::kN5,
        ...
        if ((pErrorMessage != nullptr) && pErrorMessage->empty()) {
            *pErrorMessage = "GAXSK::Bake failed while planning GSeedRunKDF3 A";
        }
        delete aAXSK;
        return false;
    }
    ...
    if (!aAXSK->Bake(GAXSFormat::kN5,
                     ...
                     pErrorMessage)) {
        if ((pErrorMessage != nullptr) && pErrorMessage->empty()) {
            *pErrorMessage = "GAXSK::Bake failed while planning GSeedRunKDF3 B";
        }
        delete aAXSK;
        return false;
    }
    ...
    if (!aAXSK->Bake(GAXSFormat::kN5,
                     ...
                     pErrorMessage)) {
        if ((pErrorMessage != nullptr) && pErrorMessage->empty()) {
            *pErrorMessage = "GAXSK::Bake failed while planning GSeedRunKDF3 C";
        }
        delete aAXSK;
        return false;
    }
...
bool GSeedRunKDF3::Build(TwistProgramBranch &pBranch,
    ...
    if (mARXSkeletonsA.size() != 6 ||
        ...
        SetError(pErrorMessage, "GSeedRunKDF3 had incorrect GAXSK skeleton counts");
        return false;
    }
    
    pBranch.AddLine("// [gseed-run-kdf3]");
    for (int i = 0; i < 8; ++i) {
    ...
    GBatch aBatch;
    aBatch.mName = "gseed_run_kdf3";
    
    //
    // A: expansion -> work
    // First 4 rounds remix source.
    //
    std::vector<CSPRNGV2Slice> aSlicesA;
    
    CSPRNGV2Slice aSliceA1;
    aSliceA1.mARXSkeleton = mARXSkeletonsA[0];
    aSliceA1.mSources.push_back(mExpansionLanes[3]);
    aSliceA1.mSources.push_back(mExpansionLanes[2]);
    aSliceA1.mSources.push_back(mExpansionLanes[1]);
    aSliceA1.mSources.push_back(mExpansionLanes[0]);
    
    aSliceA1.mDest = mWorkLanes[0];
    ...
    aSliceA2.mSources.push_back(mWorkLanes[0]);
    aSliceA2.mSources.push_back(mExpansionLanes[3]);
    aSliceA2.mSources.push_back(mExpansionLanes[2]);
    aSliceA2.mSources.push_back(mExpansionLanes[1]);
    
    aSliceA2.mDest = mWorkLanes[1];
    ...
    aSliceA3.mSources.push_back(mWorkLanes[1]);
    aSliceA3.mSources.push_back(mWorkLanes[0]);
    aSliceA3.mSources.push_back(mExpansionLanes[3]);
    aSliceA3.mSources.push_back(mExpansionLanes[2]);
    
    aSliceA3.mDest = mWorkLanes[2];
    ...
    aSliceA4.mSources.push_back(mWorkLanes[2]);
    aSliceA4.mSources.push_back(mWorkLanes[1]);
    aSliceA4.mSources.push_back(mWorkLanes[0]);
    aSliceA4.mSources.push_back(mExpansionLanes[3]);
    aSliceA4.mDest = mWorkLanes[3];
    ...
    if (!CSPRNGV2::Bake(aSlicesA,
        ...
        printf("error with CSPRNGV2::Bake KDF3 A => %s\n", aErrorText);
        return false;
    }
    ...
    if (!CSPRNGV2::Bake(aSlicesB,
                        ...
                        pErrorMessage)) {
        const char *aErrorText = (pErrorMessage != nullptr) ? pErrorMessage->c_str() : "unknown error";
        printf("error with CSPRNGV2::Bake KDF3 B => %s\n", aErrorText);
        return false;
    }
    
    //
    // C: operation -> expansion
    //
    ...
    for (int aIndex = 0; aIndex < 4; aIndex++) {
        ...
        aSlice.mDest = mExpansionLanes[static_cast<std::size_t>(aIndex)];
        aSlice.mDestWriteInverted = ((aIndex & 1) != 0);
    ...
    if (!CSPRNGV2::Bake(aSlicesC,
                        ...
                        pErrorMessage)) {
        const char *aErrorText = (pErrorMessage != nullptr) ? pErrorMessage->c_str() : "unknown error";
        printf("error with CSPRNGV2::Bake KDF3 C => %s\n", aErrorText);
        return false;
    }
