...
std::string Render(const Candidate &c, std::size_t pIndex, const Score &pScore, std::uint64_t pCases) {
    std::ostringstream s;
    s << "// GrowA candidate " << (pIndex + 1U) << " of 33\n"
    ...
    s << "void TwistExpander_" << kNames[pIndex] << "::GrowKeyA(TwistWorkSpace *pWorkSpace) {\n"
...
bool GrowAControl::SavePreview(const std::string &pPath, std::string *pErrorMessage) {
    ...
    if (!FileIO::Save(aPath, std::vector<std::uint8_t>(all.begin(), all.end()))) {
        if (pErrorMessage) *pErrorMessage = "GrowAControl failed to save " + aPath;
        return false;
    }
    const std::string folder = std::filesystem::path(aPath).parent_path().generic_string();
    for (std::size_t i = 0U; i < gRendered.size(); ++i) {
        char name[48]; std::snprintf(name, sizeof(name), "GrowKeyA_Candidate%02zu.cpp", i + 1U);
