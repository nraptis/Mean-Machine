...
static_assert(kMaximumPairDistance == 674ULL,
              "GrowA diversity denominator must match its effective fields.");
...
Candidate CandidateFromValues(
    ...
    if ((pWarningMessage != nullptr) && (aInvalidCount != 0U)) {
        *pWarningMessage =
            "GrowAControl contained " + std::to_string(aInvalidCount) +
...
bool Validate(const Candidate &pCandidate,
    ...
    if (!ValidateRoutes(pCandidate.mQuarterBases) ||
        ...
        if (pErrorMessage != nullptr) {
            *pErrorMessage =
                "GrowA routes were not balanced four-lane permutations.";
        }
        return false;
    }
    ...
    for (std::size_t aLoop = 0U; aLoop < 4U; ++aLoop) {
        if (!IsPermutation4(pCandidate.mQuarterShifts[aLoop]) ||
            ...
            if (pErrorMessage != nullptr) {
                *pErrorMessage =
                    "GrowA shifts or diffuser selections were invalid.";
            }
            return false;
        }
    ...
    if (!IsPermutation4(pCandidate.mFinalShifts) ||
        ...
        if (pErrorMessage != nullptr) {
            *pErrorMessage =
                "GrowA candidate did not use its complete preplanned "
...
std::string Render(const Candidate &c, std::size_t pIndex, const Score &pScore, std::uint64_t pCases) {
    std::ostringstream s;
    s << "    // GrowAControl candidate " << (pIndex + 1U) << " of 33\n"
    ...
    s << "    std::uint8_t *aGrowAAetherLaneA = pWorkSpace->mAetherLaneA;\n"
    ...
    for (std::size_t loop = 0U; loop < 4U; ++loop) {
        const std::string aDestination =
            std::string("aGrowAVaporLane") + Letter(loop);
        RenderLaneFold(s,
                       "aGrowAAetherLane",
                       aDestination.c_str(),
                       "S_QUARTER1",
    ...
    for (std::size_t loop = 0U; loop < 4U; ++loop) {
        const std::string aDestination =
            std::string("aGrowAShadowLane") + Letter(loop);
        RenderLaneFold(s,
                       "aGrowAVaporLane",
                       aDestination.c_str(),
                       "W_KEY1",
    ...
    s << "            std::uint32_t aFoldWord =\n";
    for (std::size_t i = 0U; i < 4U; i += 2U) {
        s << "                (static_cast<std::uint32_t>(aGrowAShadowLane" << Letter(i)
          << "[aFoldIndex" << Letter(i) << "]) << " << (static_cast<unsigned>(c.mFinalShifts[i]) * 8U) << "U) | "
          << "(static_cast<std::uint32_t>(aGrowAShadowLane" << Letter(i + 1U)
...
std::string GrowAControl::Generate(std::uint64_t pExplorationCases) {
...
bool GrowAControl::SaveValues(const std::string &pFolder,
    ...
    if (gCandidates.size() != kCandidateCount) {
        if (pErrorMessage != nullptr) {
            *pErrorMessage =
                "GrowAControl needs all 33 candidates before saving values.";
        }
        return false;
    }
    ...
    for (std::size_t i = 0U; i < gCandidates.size(); ++i) {
        ...
        if (!Validate(gCandidates[i], &aValidationError)) {
            if (pErrorMessage != nullptr) {
                *pErrorMessage =
                    "GrowAControl refused to save candidate " +
...
bool GrowAControl::LoadValues(
    const std::string &pFolder,
    std::string *pErrorMessage) {
    if (!gDidReset || !gCandidates.empty()) {
        if (pErrorMessage != nullptr) {
            *pErrorMessage =
                "GrowAControl::Reset must precede value loading.";
        }
        return false;
    }
    const std::string aFolder = FileIO::ProjectRoot(pFolder);
...
bool GrowAControl::ValidateCandidate(
    ...
    if (pCandidateIndex >= gCandidates.size()) {
        if (pErrorMessage != nullptr) {
            *pErrorMessage = "GrowAControl candidate index was out of range.";
        }
        return false;
    }
