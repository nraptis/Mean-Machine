...
void PrintScore(const char *pLabel,
                const LoopCorePatternScore11 &pScore) {
    std::printf(
        "%s{edges:%u reciprocal:%u strong:%u articulation:%u bridges:%u "
        "destination_floor:%u source_floor:%u destination_journey:%u "
        "source_journey:%u total:%u curve:%u diameter:%u}\n",
        pLabel,
...
void PrintPattern(const LoopCorePattern11 &pPattern,
                  const std::size_t pIndex) {
    std::printf("Pattern %zu\n", pIndex + 1U);
    PrintScore("    ", pPattern.mScore);
    for (const auto &aRound : pPattern.mRounds) {
        std::printf("    { %c, %c, %c }\n",
...
void PrintSummary(const LoopCorePatternFile11 &pFile) {
    std::printf("Loop core pattern asset: evaluated=%llu eligible=%llu "
                "chosen=%zu\n",
                static_cast<unsigned long long>(pFile.mEvaluatedCount),
                static_cast<unsigned long long>(pFile.mEligibleCount),
                pFile.mPatterns.size());
    PrintScore("Whole pack worst:  ", pFile.mWholeWorst);
    PrintScore("Whole pack best:   ", pFile.mWholeBest);
    PrintScore("Chosen pack worst: ", pFile.mChosenWorst);
    PrintScore("Chosen pack best:  ", pFile.mChosenBest);
}

} // namespace

bool LoopCorePatternFarmer::Load(const std::string &pPath,
    ...
    *pFile = {};
    ...
    /*
    std::printf("Loaded loop core pattern asset %s\n", aPath.c_str());
    PrintSummary(*pFile);
    const std::size_t aPrintCount = std::min<std::size_t>(
        5U,
        pFile->mPatterns.size());
    std::printf("First %zu loaded loop core patterns:\n", aPrintCount);
    for (std::size_t i = 0U; i < aPrintCount; ++i) {
        PrintPattern(pFile->mPatterns[i], i);
    }
    */
    return true;
}
