#include "TwistExpander_CrooooNNnnNN.hpp"
#include "TwistFunctional.hpp"
#include "TwistMasking.hpp"
#include "TwistMix64.hpp"
#include "TwistSnow.hpp"
#include "TwistMemory.hpp"
#include "TwistCryptoGenerator.hpp"

#include <cstring>

const std::uint8_t TwistExpander_CrooooNNnnNN::kSBoxA[S_SBOX] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSBoxB[S_SBOX] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSBoxC[S_SBOX] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSBoxD[S_SBOX] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSaltA[S_SALT] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSaltB[S_SALT] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSaltC[S_SALT] = {
    
};

const std::uint8_t TwistExpander_CrooooNNnnNN::kSaltD[S_SALT] = {
    
};

TwistExpander_CrooooNNnnNN::TwistExpander_CrooooNNnnNN()
: TwistExpander() {
    std::memcpy(mSBoxA, kSBoxA, sizeof(mSBoxA));
    std::memcpy(mSBoxB, kSBoxB, sizeof(mSBoxB));
    std::memcpy(mSBoxC, kSBoxC, sizeof(mSBoxC));
    std::memcpy(mSBoxD, kSBoxD, sizeof(mSBoxD));
    std::memcpy(mSaltA, kSaltA, sizeof(mSaltA));
    std::memcpy(mSaltB, kSaltB, sizeof(mSaltB));
    std::memcpy(mSaltC, kSaltC, sizeof(mSaltC));
    std::memcpy(mSaltD, kSaltD, sizeof(mSaltD));
}

void TwistExpander_CrooooNNnnNN::Seed(TwistWorkSpace *pWorkspace,
                                 std::uint8_t *pSource,
                                 std::uint8_t *pPassword,
                                 unsigned int pPasswordByteLength) {
    TwistExpander::Seed(pWorkspace, pSource, pPassword, pPasswordByteLength);
    if (pWorkspace == nullptr) { return; }
    [[maybe_unused]] std::uint8_t *aExpandLaneA = pWorkspace->mExpandLaneA;
    [[maybe_unused]] std::uint8_t *aExpandLaneB = pWorkspace->mExpandLaneB;
    [[maybe_unused]] std::uint8_t *aWorkLaneA = pWorkspace->mWorkLaneA;
    [[maybe_unused]] std::uint8_t *aWorkLaneB = pWorkspace->mWorkLaneB;
    [[maybe_unused]] std::uint8_t *aWorkLaneC = pWorkspace->mWorkLaneC;
    [[maybe_unused]] std::uint8_t *aWorkLaneD = pWorkspace->mWorkLaneD;
    std::uint64_t aValuePrevious = 0;
    std::uint64_t e = 0;
    std::uint64_t sec_a = 0;
    std::uint64_t sec_b = 0;
    std::uint64_t cross = 0;
    std::uint64_t a = 0;
    std::uint64_t s0 = 0;
    std::uint64_t b = 0;
    std::uint64_t s1 = 0;
    std::uint64_t c = 0;
    std::uint64_t s2 = 0;
    std::uint64_t d = 0;
    std::uint64_t carry = 0;
    std::uint64_t aValueMix = 0;
    std::uint64_t aValue = 0;
    std::uint64_t aSelect = 0;

    // Making snow counter: sha 256

    TwistSnow::Sha256Counter(pSource, aExpandLaneA);
    TwistSnow::Sha256Counter(aExpandLaneA, aExpandLaneB);

    TwistMemory::Grow(mScratchSaltA, mSaltB, 128U);
    TwistMemory::Grow(mScratchSaltB, mSaltD, 128U);
    TwistMemory::Grow(mScratchSaltD, mSaltC, 128U);
    TwistMemory::Grow(mScratchSaltC, mSaltA, 128U);

    for (std::uint16_t aIndex = 0U; aIndex < S_BLOCK; aIndex += 1U) {
        aValuePrevious = e;
        sec_a = aExpandLaneA[aIndex];
        sec_b = aExpandLaneB[S_BLOCK1 - aIndex];
        cross = aExpandLaneB[aIndex];
        e = sec_b;
        // __START_CSPRNG
        // a = s0 + e + hot0.add;
        a = s0 + RotL64(e, 11U) + 7264055884651018793ULL;
        // b = s1 ^ previous ^ hot1.add;
        b = s1 ^ RotL64(aValuePrevious, 27U) ^ 2950228849917044260ULL;
        // c = s2 + cross + fixedSaltA + scratchSaltA;
        c = s2 + RotL64(cross, 37U) + mSaltB[((127U - aIndex + 80U) & 0x7FU)] + mScratchSaltA[((aIndex + 64U) & 0x7FU)];
        // d = carry ^ fixedSaltB ^ scratchSaltB ^ RotL64(e, r0);
        d = RotL64(carry, 43U) ^ RotL64(e, 21U) ^ mSaltD[((127U - aIndex + 71U) & 0x7FU)] ^ mScratchSaltB[((aIndex + 8U) & 0x7FU)];
        // a += b;
        // d ^= a;
        // d = RotL64(d, r1);
        a = a + b + mScratchSaltA[((carry + sec_b + 93U) & 0x7FU)];
        d = d ^ a ^ 13397864124185702106ULL;
        d = RotL64(d, 29U);
        // c += d;
        // b ^= c;
        // b = RotL64(b, r2);
        c = c + d;
        b = b ^ c ^ mScratchSaltD[(((cross ^ sec_a) + 11U) & 0x7FU)];
        b = RotL64(b, 51U);
        // a += b + hot4.add;
        // d ^= a;
        // d *= hot4.mul;
        // d = RotL64(d, r3);
        a = a + b + 3360240144980893918ULL;
        d = d ^ a;
        d = d * 14938148063424955233ULL;
        d = RotL64(d, 35U);
        // c += d + hot4.add;
        // b ^= c;
        // b *= hot4.mul;
        // b = RotL64(b, r4);
        c = c + d + 8054982038916672949ULL;
        b = b ^ c;
        b = b * 17772899028324819669ULL;
        b = RotL64(b, 11U);
        // Chain up "p" main combine expression
        e = (RotL64(a, 27U) ^ d) + (b ^ RotL64(c, 21U));
        aValueMix = TwistMix64::Diffuse(ShiftL64(sec_a, 32U) ^ sec_b ^ RotL64(aValuePrevious, 29U));
        // Push "p" through secure byte diffusion word
        e = e + aValueMix;
        e = TwistMix64::Diffuse(e);
        aWorkLaneA[aIndex] = e;
        // Pass "p" through prismatic gate
        aValue = e;
        aValue = aValue ^ RotL64(s1, 37U);
        aValue = aValue ^ a ^ RotL64(d, 11U);
        aValue = TwistMix64::Diffuse(aValue);
        aValue = aValue + RotL64(sec_b, 3U);
        aValue = TwistMix64::Diffuse(aValue);
        aSelect = aValue;
        if ((aSelect & 0x14U) > 8) {
        	if ((aSelect & 0x14U) > 17) {
        		e = TwistMix64::GateRollC_4_8(e, 53ULL, mSBoxA, mSBoxD, mSBoxB, mSBoxC);
        	} else {
        		e = TwistMix64::GateTurnB_4_8(e, 13ULL, mSBoxA, mSBoxD, mSBoxB, mSBoxC);
        	}
        } else {
        	if ((aSelect & 0x14U) > 1) {
        		e = TwistMix64::GateTurnC_4_8(e, 19ULL, mSBoxA, mSBoxD, mSBoxB, mSBoxC);
        	} else {
        		e = TwistMix64::GateTurnA_4_8(e, 45ULL, mSBoxA, mSBoxD, mSBoxB, mSBoxC);
        	}
        }
        // s0 += e ^ previous ^ d;
        // s0 *= hot0.mul;
        // s0 = RotL64(s0, r3);
        s0 = s0 + (e ^ aValuePrevious ^ d);
        s0 = s0 + mSaltB[((aValuePrevious + cross + 54U) & 0x7FU)];
        s0 = s0 * 7720829276422159377ULL;
        s0 = RotL64(s0, 45U);
        // s1 ^= e + cross + a;
        // s1 *= hot1.mul;
        // s1 = RotL64(s1, r4);
        s1 = s1 ^ (e + cross + a) ^ mScratchSaltA[(((e ^ sec_b) + 92U) & 0x7FU)];
        s1 = s1 * 7090949039097081517ULL;
        s1 = RotL64(s1, 43U);
        // s2 += b ^ c ^ fixedSalt;
        // s2 *= hot2.mul;
        // s2 = RotL64(s2, r5);
        s2 = s2 + (b ^ c ^ mSaltD[((aIndex + 7U) & 0x7FU)]);
        s2 = s2 * 5392141865415645607ULL;
        s2 = RotL64(s2, 19U);
        // carry += s0 ^ s1 ^ s2 ^ e;
        // carry *= hot7.mul;
        // carry = RotL64(carry, r6);
        // carry ^= carry >> rshift;
        carry = carry + (s0 ^ s1 ^ s2 ^ e);
        carry = carry * 4317237461525766693ULL;
        carry = RotL64(carry, 51U);
        carry = carry ^ ShiftR64(carry, 29U);
        // __END_CSPRNG
    }

    // Secure zero on crypto buffer.

    TwistMemory::ZeroBlock(aExpandLaneA);
    TwistMemory::ZeroBlock(aExpandLaneB);

    // Making snow counter: cha cha 20

    TwistSnow::ChaCha20Counter(pSource, aExpandLaneA);
    TwistSnow::ChaCha20Counter(aExpandLaneA, aExpandLaneB);

    TwistMemory::Grow(mScratchSaltB, mSaltD, 128U);
    TwistMemory::Grow(mScratchSaltD, mSaltC, 128U);
    TwistMemory::Grow(mScratchSaltC, mSaltA, 128U);
    TwistMemory::Grow(mScratchSaltA, mSaltB, 128U);

    for (std::uint16_t aIndex = 0U; aIndex < S_BLOCK; aIndex += 1U) {
        aValuePrevious = e;
        sec_a = aExpandLaneB[S_BLOCK1 - aIndex];
        sec_b = aExpandLaneA[aIndex];
        cross = aExpandLaneB[aIndex];
        e = sec_a;
        // __START_CSPRNG
        // a = s0 + e + hot0.add;
        a = s0 + RotL64(e, 27U) + 4714254121773480761ULL;
        // b = s1 ^ previous ^ hot1.add;
        b = s1 ^ RotL64(aValuePrevious, 43U) ^ 12719774865051505415ULL;
        // c = s2 + cross + fixedSaltA + scratchSaltA;
        c = s2 + RotL64(cross, 13U) + mSaltD[((aIndex + 82U) & 0x7FU)] + mScratchSaltB[((127U - aIndex + 62U) & 0x7FU)];
        // d = carry ^ fixedSaltB ^ scratchSaltB ^ RotL64(e, r0);
        d = RotL64(carry, 51U) ^ RotL64(e, 45U) ^ mSaltC[((aIndex + 18U) & 0x7FU)] ^ mScratchSaltD[((127U - aIndex + 34U) & 0x7FU)];
        // a += b;
        // d ^= a;
        // d = RotL64(d, r1);
        a = a + b + mScratchSaltD[((carry + sec_b + 95U) & 0x7FU)];
        d = d ^ a ^ 16041310825161308530ULL;
        d = RotL64(d, 35U);
        // c += d;
        // b ^= c;
        // b = RotL64(b, r2);
        c = c + d + 17983621439010993981ULL;
        b = b ^ c ^ mScratchSaltD[(((aValue ^ sec_a) + 92U) & 0x7FU)];
        b = RotL64(b, 21U);
        // a += b + hot4.add;
        // d ^= a;
        // d *= hot4.mul;
        // d = RotL64(d, r3);
        a = a + b + 9877214802443864835ULL;
        d = d ^ a;
        d = d * 14956561894254835193ULL;
        d = RotL64(d, 29U);
        // c += d + hot4.add;
        // b ^= c;
        // b *= hot4.mul;
        // b = RotL64(b, r4);
        c = c + d + 10554263146660944689ULL;
        b = b ^ c;
        b = b * 4301372799565802491ULL;
        b = RotL64(b, 53U);
        // Chain up "p" main combine expression
        e = (a ^ RotL64(b, 19U)) + (c ^ RotL64(d, 27U));
        aValueMix = TwistMix64::Diffuse(ShiftL64(sec_a, 32U) ^ sec_b ^ RotL64(aValuePrevious, 21U));
        // Push "p" through secure byte diffusion word
        e = e + aValueMix;
        e = TwistMix64::Diffuse(e);
        aWorkLaneB[S_BLOCK1 - aIndex] = e;
        // Pass "p" through prismatic gate
        aValue = e;
        aValue = aValue ^ RotL64(s1, 43U);
        aValue = aValue ^ RotL64(c, 29U) ^ d;
        aValue = TwistMix64::Diffuse(aValue);
        aValue = aValue + RotL64(sec_b, 7U);
        aValue = TwistMix64::Diffuse(aValue);
        aSelect = aValue;
        if ((aSelect & 0x28U) > 24) {
        	if ((aSelect & 0x28U) > 33) {
        		e = TwistMix64::GateRollA_4_8(e, 45ULL, mSBoxD, mSBoxB, mSBoxC, mSBoxA);
        	} else {
        		e = TwistMix64::GateRollC_4_8(e, 13ULL, mSBoxD, mSBoxB, mSBoxC, mSBoxA);
        	}
        } else {
        	if ((aSelect & 0x28U) > 2) {
        		e = TwistMix64::GateTurnB_4_8(e, 11ULL, mSBoxD, mSBoxB, mSBoxC, mSBoxA);
        	} else {
        		e = TwistMix64::GateTurnC_4_8(e, 45ULL, mSBoxD, mSBoxB, mSBoxC, mSBoxA);
        	}
        }
        // s0 += e ^ previous ^ d;
        // s0 *= hot0.mul;
        // s0 = RotL64(s0, r3);
        s0 = s0 + (e ^ aValuePrevious ^ d);
        s0 = s0 + mSaltD[((carry + sec_a + 87U) & 0x7FU)];
        s0 = s0 * 7013418997134973357ULL;
        s0 = RotL64(s0, 51U);
        // s1 ^= e + cross + a;
        // s1 *= hot1.mul;
        // s1 = RotL64(s1, r4);
        s1 = s1 ^ (e + cross + a) ^ mScratchSaltA[((cross + sec_a + 43U) & 0x7FU)];
        s1 = s1 * 4439612937210029533ULL;
        s1 = RotL64(s1, 19U);
        // s2 += b ^ c ^ fixedSalt;
        // s2 *= hot2.mul;
        // s2 = RotL64(s2, r5);
        s2 = s2 + (b ^ c ^ mSaltD[((127U - aIndex + 1U) & 0x7FU)]);
        s2 = s2 * 1092826363616991607ULL;
        s2 = RotL64(s2, 27U);
        // carry += s0 ^ s1 ^ s2 ^ e;
        // carry *= hot7.mul;
        // carry = RotL64(carry, r6);
        // carry ^= carry >> rshift;
        carry = carry + (s0 ^ s1 ^ s2 ^ e);
        carry = carry * 334659368971890877ULL;
        carry = RotL64(carry, 35U);
        carry = carry ^ ShiftR64(carry, 41U);
        // __END_CSPRNG
    }

    // Secure zero on crypto buffer.

    TwistMemory::ZeroBlock(aExpandLaneA);
    TwistMemory::ZeroBlock(aExpandLaneB);

    // Making snow counter: aes 256

    TwistSnow::AES256Counter(pSource, aExpandLaneA);
    TwistSnow::AES256Counter(aExpandLaneA, aExpandLaneB);

    TwistMemory::Grow(mScratchSaltC, mSaltC, 128U);
    TwistMemory::Grow(mScratchSaltA, mSaltA, 128U);
    TwistMemory::Grow(mScratchSaltB, mSaltB, 128U);
    TwistMemory::Grow(mScratchSaltD, mSaltD, 128U);

    for (std::uint16_t aIndex = 0U; aIndex < S_BLOCK; aIndex += 1U) {
        aValuePrevious = e;
        sec_a = aExpandLaneB[S_BLOCK1 - aIndex];
        sec_b = aExpandLaneA[S_BLOCK1 - aIndex];
        cross = aExpandLaneB[aIndex];
        e = sec_a;
        // __START_CSPRNG
        // a = s0 + e + hot0.add;
        a = s0 + RotL64(e, 35U) + 12191915750254584983ULL;
        // b = s1 ^ previous ^ hot1.add;
        b = s1 ^ RotL64(aValuePrevious, 53U) ^ 131582210200048615ULL;
        // c = s2 + cross + fixedSaltA + scratchSaltA;
        c = s2 + RotL64(cross, 11U) + mSaltC[((aIndex + 117U) & 0x7FU)] + mScratchSaltC[((127U - aIndex + 42U) & 0x7FU)];
        // d = carry ^ fixedSaltB ^ scratchSaltB ^ RotL64(e, r0);
        d = RotL64(carry, 43U) ^ RotL64(e, 19U) ^ mSaltA[((aIndex + 25U) & 0x7FU)] ^ mScratchSaltA[((127U - aIndex + 39U) & 0x7FU)];
        // a += b;
        // d ^= a;
        // d = RotL64(d, r1);
        a = a + b + 15596070237865826066ULL;
        d = d ^ a ^ mScratchSaltC[(((carry ^ sec_a) + 30U) & 0x7FU)];
        d = RotL64(d, 45U);
        // c += d;
        // b ^= c;
        // b = RotL64(b, r2);
        c = c + d + mScratchSaltA[(((aValue ^ sec_a) + 51U) & 0x7FU)] + 3054500413081257597ULL;
        b = b ^ c;
        b = RotL64(b, 21U);
        // a += b + hot4.add;
        // d ^= a;
        // d *= hot4.mul;
        // d = RotL64(d, r3);
        a = a + b + 15178064206279447263ULL;
        d = d ^ a;
        d = d * 18012283899262758641ULL;
        d = RotL64(d, 13U);
        // c += d + hot4.add;
        // b ^= c;
        // b *= hot4.mul;
        // b = RotL64(b, r4);
        c = c + d + 16049149428947630314ULL;
        b = b ^ c;
        b = b * 13702664255233372033ULL;
        b = RotL64(b, 53U);
        // Chain up "p" main combine expression
        e = (a ^ RotL64(b, 19U)) + (c ^ RotL64(d, 13U));
        aValueMix = TwistMix64::Diffuse(sec_a ^ ShiftL64(sec_b, 32U) ^ RotL64(aValuePrevious, 19U));
        // Push "p" through secure byte diffusion word
        e = e ^ aValueMix;
        e = TwistMix64::Diffuse(e);
        aWorkLaneC[aIndex] = e;
        // Pass "p" through prismatic gate
        aValue = e;
        aValue = aValue ^ RotL64(s1, 45U);
        aValue = aValue ^ a ^ RotL64(b, 13U);
        aValue = TwistMix64::Diffuse(aValue);
        aValue = aValue ^ RotL64(sec_b, 19U);
        aValue = TwistMix64::Diffuse(aValue);
        aSelect = aValue;
        if ((aSelect & 0x36U) > 27) {
        	e = TwistMix64::GateTurnB_4_8(e, 35ULL, mSBoxC, mSBoxA, mSBoxD, mSBoxB);
        } else {
        	e = TwistMix64::GateRollC_4_8(e, 11ULL, mSBoxC, mSBoxA, mSBoxD, mSBoxB);
        }
        // s0 += e ^ previous ^ d;
        // s0 *= hot0.mul;
        // s0 = RotL64(s0, r3);
        s0 = s0 + (e ^ aValuePrevious ^ d);
        s0 = s0 + mSaltC[((cross + sec_a + 60U) & 0x7FU)];
        s0 = s0 * 10497303007298788949ULL;
        s0 = RotL64(s0, 53U);
        // s1 ^= e + cross + a;
        // s1 *= hot1.mul;
        // s1 = RotL64(s1, r4);
        s1 = s1 ^ (e + cross + a) ^ mScratchSaltD[((a + cross + 104U) & 0x7FU)];
        s1 = s1 * 5810598635849069251ULL;
        s1 = RotL64(s1, 45U);
        // s2 += b ^ c ^ fixedSalt;
        // s2 *= hot2.mul;
        // s2 = RotL64(s2, r5);
        s2 = s2 + (b ^ c ^ mSaltC[((aIndex + 89U) & 0x7FU)]);
        s2 = s2 * 4721763854098020963ULL;
        s2 = RotL64(s2, 35U);
        // carry += s0 ^ s1 ^ s2 ^ e;
        // carry *= hot7.mul;
        // carry = RotL64(carry, r6);
        // carry ^= carry >> rshift;
        carry = carry + (s0 ^ s1 ^ s2 ^ e);
        carry = carry * 4057546188715483405ULL;
        carry = RotL64(carry, 11U);
        carry = carry ^ ShiftR64(carry, 29U);
        // __END_CSPRNG
    }

    // Secure zero on crypto buffer.

    TwistMemory::ZeroBlock(aExpandLaneA);
    TwistMemory::ZeroBlock(aExpandLaneB);

    // Making snow counter: aria 256

    TwistSnow::Aria256Counter(pSource, aExpandLaneA);
    TwistSnow::Aria256Counter(aExpandLaneA, aExpandLaneB);

    TwistMemory::Grow(mScratchSaltD, mSaltA, 128U);
    TwistMemory::Grow(mScratchSaltC, mSaltB, 128U);
    TwistMemory::Grow(mScratchSaltA, mSaltD, 128U);
    TwistMemory::Grow(mScratchSaltB, mSaltC, 128U);

    for (std::uint16_t aIndex = 0U; aIndex < S_BLOCK; aIndex += 1U) {
        aValuePrevious = e;
        sec_a = aExpandLaneA[S_BLOCK1 - aIndex];
        sec_b = aExpandLaneB[aIndex];
        cross = aExpandLaneA[aIndex];
        e = sec_a;
        // __START_CSPRNG
        // a = s0 + e + hot0.add;
        a = s0 + RotL64(e, 21U) + 4581456012075548581ULL;
        // b = s1 ^ previous ^ hot1.add;
        b = s1 ^ RotL64(aValuePrevious, 29U) ^ 7092756233861150478ULL;
        // c = s2 + cross + fixedSaltA + scratchSaltA;
        c = s2 + RotL64(cross, 37U) + mSaltA[((aIndex + 1U) & 0x7FU)] + mScratchSaltD[((127U - aIndex + 91U) & 0x7FU)];
        // d = carry ^ fixedSaltB ^ scratchSaltB ^ RotL64(e, r0);
        d = RotL64(carry, 27U) ^ RotL64(e, 29U) ^ mSaltB[((aIndex + 81U) & 0x7FU)] ^ mScratchSaltC[((127U - aIndex + 33U) & 0x7FU)];
        // a += b;
        // d ^= a;
        // d = RotL64(d, r1);
        a = a + b;
        d = d ^ a ^ mScratchSaltC[(((aValuePrevious ^ sec_b) + 94U) & 0x7FU)] ^ 4896071878102567566ULL;
        d = RotL64(d, 51U);
        // c += d;
        // b ^= c;
        // b = RotL64(b, r2);
        c = c + d;
        b = b ^ c ^ mScratchSaltB[(((cross ^ sec_a) + 79U) & 0x7FU)];
        b = RotL64(b, 29U);
        // a += b + hot4.add;
        // d ^= a;
        // d *= hot4.mul;
        // d = RotL64(d, r3);
        a = a + b + 7068437165967667586ULL;
        d = d ^ a;
        d = d * 7471158782583781535ULL;
        d = RotL64(d, 37U);
        // c += d + hot4.add;
        // b ^= c;
        // b *= hot4.mul;
        // b = RotL64(b, r4);
        c = c + d + 18271038308613319037ULL;
        b = b ^ c;
        b = b * 3412318732803378973ULL;
        b = RotL64(b, 27U);
        // Chain up "p" main combine expression
        e = (RotL64(a, 11U) ^ b) + (RotL64(c, 27U) ^ d);
        aValueMix = TwistMix64::Diffuse(sec_a ^ ShiftL64(sec_b, 32U) ^ RotL64(aValuePrevious, 21U));
        // Push "p" through secure byte diffusion word
        e = e + aValueMix;
        e = TwistMix64::Diffuse(e);
        aWorkLaneD[S_BLOCK1 - aIndex] = e;
        // Pass "p" through prismatic gate
        aValue = e;
        aValue = aValue ^ RotL64(s0, 51U);
        aValue = RotL64(aValue, 29U) ^ c ^ d;
        aValue = TwistMix64::Diffuse(aValue);
        aValue = aValue + RotL64(sec_a, 11U);
        aValue = TwistMix64::Diffuse(aValue);
        aSelect = aValue;
        if ((aSelect & 0x0AU) > 2) {
        	if ((aSelect & 0x0AU) > 9) {
        		e = TwistMix64::GateRollB_4_8(e, 27ULL, mSBoxB, mSBoxC, mSBoxA, mSBoxD);
        	} else {
        		e = TwistMix64::GateRollA_4_8(e, 37ULL, mSBoxB, mSBoxC, mSBoxA, mSBoxD);
        	}
        } else {
        	if ((aSelect & 0x0AU) > 1) {
        		e = TwistMix64::GateRollB_4_8(e, 29ULL, mSBoxB, mSBoxC, mSBoxA, mSBoxD);
        	} else {
        		e = TwistMix64::GateRollA_4_8(e, 51ULL, mSBoxB, mSBoxC, mSBoxA, mSBoxD);
        	}
        }
        // s0 += e ^ previous ^ d;
        // s0 *= hot0.mul;
        // s0 = RotL64(s0, r3);
        s0 = s0 + (e ^ aValuePrevious ^ d);
        s0 = s0 + mSaltA[(((cross ^ sec_b) + 126U) & 0x7FU)];
        s0 = s0 * 1460876498030652875ULL;
        s0 = RotL64(s0, 27U);
        // s1 ^= e + cross + a;
        // s1 *= hot1.mul;
        // s1 = RotL64(s1, r4);
        s1 = s1 ^ (e + cross + a) ^ mScratchSaltD[((e + sec_a + 17U) & 0x7FU)];
        s1 = s1 * 16738016723078405111ULL;
        s1 = RotL64(s1, 51U);
        // s2 += b ^ c ^ fixedSalt;
        // s2 *= hot2.mul;
        // s2 = RotL64(s2, r5);
        s2 = s2 + (b ^ c ^ mSaltA[((127U - aIndex + 98U) & 0x7FU)]);
        s2 = s2 * 8341919074424203667ULL;
        s2 = RotL64(s2, 29U);
        // carry += s0 ^ s1 ^ s2 ^ e;
        // carry *= hot7.mul;
        // carry = RotL64(carry, r6);
        // carry ^= carry >> rshift;
        carry = carry + (s0 ^ s1 ^ s2 ^ e);
        carry = carry * 15621892292900496015ULL;
        carry = RotL64(carry, 35U);
        carry = carry ^ ShiftR64(carry, 27U);
        // __END_CSPRNG
    }

    // Secure zero on crypto buffer.

    TwistMemory::ZeroBlock(aExpandLaneA);
    TwistMemory::ZeroBlock(aExpandLaneB);

    // Secure Zero Out

    TwistMemory::ZeroBlock(aExpandLaneA);

    // Secure Zero Out

    TwistMemory::ZeroBlock(aExpandLaneB);

}

void TwistExpander_CrooooNNnnNN::TwistBlock(TwistWorkSpace *pWorkspace,
                                       std::uint8_t *pSource,
                                       std::uint8_t *pDestination) {
    TwistExpander::TwistBlock(pWorkspace, pSource, pDestination);
    if ((pWorkspace == nullptr) || (pDestination == nullptr)) { return; }
    std::memcpy(pDestination, pWorkspace->mWorkLaneD, S_BLOCK);
}
